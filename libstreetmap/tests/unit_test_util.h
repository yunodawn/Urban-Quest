...
template< typename Expected, typename Actual, typename Tolerance >
void CheckAbsoluteError(UnitTest::TestResults& results, Expected const& expected, Actual const& actual, Tolerance const& tolerance,
                UnitTest::TestDetails const& details, const char* stringified_actual)
{
    const auto abs_error = absolute_error(expected, actual);
    if (!UnitTest::AreClose(0.0, abs_error, tolerance))
    {
        UnitTest::MemoryOutStream stream;
        stream << "Expected (" << expected << ") and actual (" << actual << ") should be within "
            << tolerance << ", but were " << abs_error << " different. For expression: " << stringified_actual;
...
#ifdef ECE297_CHECK_ABSOLUTE_ERROR
    #error unit_test_util.h redefines ECE297_CHECK_ABSOLUTE_ERROR
#endif
...
#define ECE297_CHECK_ABSOLUTE_ERROR(expected, actual, tolerance) \
    do \
    { \
        try { \
            ece297test::CheckAbsoluteError(*UnitTest::CurrentTest::Results(), expected, actual, tolerance, UnitTest::TestDetails(*UnitTest::CurrentTest::Details(), __LINE__), #actual); \
        } \
        catch (...) { \
            UnitTest::CurrentTest::Results()->OnTestFailure(UnitTest::TestDetails(*UnitTest::CurrentTest::Details(), __LINE__), \
                    "Unhandled exception in ECE297_CHECK_ABSOLUTE_ERROR(" #expected ", " #actual ", " #tolerance ")"); \
        } \
    } while (0)
